...
export function ExecutionDetailPage() {
  ...
  const load = () => {
    ...
    (executionsApi.get(id) as Promise<{ data: { data: Execution } }>)
      .then((r) => {
        ...
        const active = (ex as Execution).stepExecutions.find((s) => s.status === 'ACTIVE');
  ...
  const handleComplete = async () => {
    ...
    setSubmitting(true);
    try {
      await executionsApi.completeStep(id, activeStep.stepId, { data: formData });
      setFormData({});
      load();
    } finally {
      setSubmitting(false);
  ...
  const handleReject = async () => {
    if (!id || !activeStep || !rejectReason.trim()) return;
    setSubmitting(true);
    try {
      await executionsApi.rejectStep(id, activeStep.stepId, { reason: rejectReason });
      setRejectReason('');
      load();
    } finally {
      setSubmitting(false);
  ...
  const handleCancel = async () => {
    if (!id || !confirm('Cancel this execution?')) return;
    await executionsApi.cancel(id, 'Cancelled by user');
    navigate('/executions');
  };
  ...
  if (!execution) return <div className="card text-center text-gray-500">Execution not found.</div>;
    ...
    <div className="space-y-6 max-w-3xl">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <span className={`px-3 py-1 rounded-full text-sm font-medium ${statusColor[execution.status] ?? 'bg-gray-100'}`}>{execution.status}</span>
          {execution.status === 'RUNNING' && (
            <button onClick={handleCancel} className="btn-secondary text-sm text-red-600">Cancel</button>
          )}
        </div>
      </div>
      ...
      <div className="space-y-3">
        {sorted.map((se, i) => (
          <div key={se.id} className={`card border-l-4 ${
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-400 uppercase">{se.step.type}</p>
                </div>
              </div>
              <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${statusColor[se.status] ?? 'bg-gray-100'}`}>{se.status}</span>
            </div>
              ...
              <div className="mt-4 space-y-3 pt-3 border-t border-gray-100">
                ...
                {(se.step.type === 'APPROVAL' || se.step.type === 'REVIEW') && (
                  <div className="space-y-2">
                    ...
                    <div className="flex gap-2">
                      <button
                        onClick={() => { setFormData((d) => ({ ...d, approved: 'true', reviewed: 'true' })); handleComplete(); }}
                        disabled={submitting}
                      ...
                      <div className="flex gap-2 flex-1">
                        ...
                        />
                        <button onClick={handleReject} disabled={submitting} className="btn-secondary text-red-600 text-sm">Reject</button>
                      </div>
                    </div>
                  </div>
                )}
                ...
                {se.step.type === 'TASK' && (
                  <button
                    onClick={() => { setFormData((d) => ({ ...d, completed: 'true' })); handleComplete(); }}
                    disabled={submitting}

  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button className="btn-primary" onClick={() => setCreating(true)}>+ New Process</button>
      </div>
            ...
            <div key={p.id} className="card flex items-center justify-between">
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-gray-400 mt-1">
                  {p._count?.steps ?? 0} steps · {p._count?.executions ?? 0} executions
                </p>
              </div>
              <div className="flex items-center gap-2 ml-4">
                <Link to={`/processes/${p.id}/builder`} className="btn-secondary text-sm">Edit</Link>
                {p.status === 'DRAFT' && (
                  <button onClick={() => handlePublish(p.id)} className="btn-primary text-sm">Publish</button>
                )}
                {p.status === 'ACTIVE' && (
                  <Link to={`/executions/new?processId=${p.id}`} className="btn-primary text-sm">Run</Link>
                )}
                <button onClick={() => handleDelete(p.id)} className="text-red-500 hover:text-red-700 text-sm px-2">Delete</button>
              </div>
            </div>
          ))}
        </div>
